...
public class ManagmentCart {
    ...
    public void insertItem(ItemsModel item) {
        ...
        if (existAlready) {
            listItem.get(n).setNumberingCart(item.getNumberingCart());
        } else {
            listItem.add(item);
        }
    ...
    public void minusItem(ArrayList<ItemsModel> listItem, int position, ChangeNumberItemsListener changeNumberItemsListener) {
        if (listItem.get(position).getNumberingCart() == 1) {
            listItem.remove(position);
        } else {
            listItem.get(position).setNumberingCart(listItem.get(position).getNumberingCart() - 1);
        }
        tinyDB.putListObject("CartList", listItem);
    ...
    public Double getTotalFee() {
        ...
        for (int i = 0; i < listItem2.size(); i++) {
            fee = fee + (listItem2.get(i).getPrice() * listItem2.get(i).getNumberingCart());
        }
        return fee;
    }
}
